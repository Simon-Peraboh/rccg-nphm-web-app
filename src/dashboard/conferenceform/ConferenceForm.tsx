...
const ConferenceForm: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<ConferenceFormInputs> = async (data) => {
    try {
        ...
        if (axios.isAxiosError(error)) {
            if (error.response) {
                console.error("📡 Full Server Response:", error.response); // ✅ Log full response
                
                const { status, data } = error.response; // ✅ Extract status & data

                if (status === 422 && data.errors) {
                    console.log("⚠️ Validation Errors:", data.errors);
  ...
  return (
    <div className="container mx-auto mt-10">
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-md p-8 mx-auto bg-white rounded shadow-md">
        <h2 className="mb-6 text-2xl font-bold">Conference Registration</h2>
