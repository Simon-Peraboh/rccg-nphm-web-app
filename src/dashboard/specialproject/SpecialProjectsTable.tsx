...
const SpecialProjectsTable: React.FC = () => {
  const [reports, setReports] = useState<SpecialProjects[]>([]);
    ...
    <div className="overflow-x-auto mr-1 ml-1">
      <div className="flex justify-between items-center mb-4 p-2 rounded-xl">
        ...
        <div className="flex space-x-2">
          <Link to="/dashboard/specialProjectsCreate" className="p-2 bg-gray-400 text-white rounded text-xl">Create Report</Link>
          <button
            ...
            <tbody>
              {currentReports.map((report, index) => (
                <tr key={report.id} className="border-b-2 border-gray-400 hover:bg-white">
                  ...
                  <td className="px-2 py-1">{report.projectName}</td>
                  <td className="px-2 py-1">{report.projectDescription}</td>
                  <td className="px-2 py-1">{report.projectLocation}</td>
                  <td className="px-2 py-1">{report.state}</td>
                  <td className="px-2 py-1">{report.projectCost}</td>
                  <td className="px-2 py-1">{report.projectManager}</td>
                  <td className="px-2 py-1">{report.projectCompletedDate}</td>
                  <td className="px-2 py-1 flex space-x-2 justify-center">
                    <Link to={`/dashboard/specialProjectsView/${report.id}`} className="text-blue-500 hover:text-blue-700">
                      <FaEye />
                    </Link>
