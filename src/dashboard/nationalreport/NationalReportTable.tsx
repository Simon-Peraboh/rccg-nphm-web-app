...
const NationalReportTable: React.FC = () => {
  ...
  const fetchReports = async () => {
    try {
      const response = await axios.get<NationalReport[]>("https://nphmapp.rccgphm.org/api/nationalReport/getAllReport");
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
              await axios.delete(`https://nphmapp.rccgphm.org/api/nationalReport/deleteReport/${id}`);
  ...
  const exportToExcel = (exportAll: boolean = false) => {
    ...
    XLSX.writeFile(wb, "NationalReports.xlsx");
  };
    ...
    <div className="overflow-x-auto mr-1 ml-1">
      <div className="flex justify-between items-center mb-4 p-2 rounded-xl">
        ...
        <div className="flex space-x-2">
          <Link to="/dashboard/nationalReportCreate" className="p-2 bg-gray-400 text-white rounded text-xl">Create Report</Link>
          <button
          ...
          <table className="min-w-full table-auto bg-cyan-50 rounded-md shadow-md">
            <thead className="bg-gray-200">
              <tr>
                ...
                <th className="px-2 py-1">Core Duties</th>
                <th className="px-2 py-1">Monthly Task</th>
                <th className="px-2 py-1">Task Done</th>
                <th className="px-2 py-1">Budgeted</th>
                <th className="px-2 py-1">Actual</th>
                <th className="px-2 py-1">Strength</th>
                <th className="px-2 py-1">Weakness</th>
